...
if [ ${#tests[@]} -eq 0 ]; then
    cp -R "/home/user/-f2fs/oe/build.OE-Generic.x86_64-9.0-devel/re2c-1.0.3/test"/* "/home/user/-f2fs/oe/build.OE-Generic.x86_64-9.0-devel/re2c-1.0.3/examples"/* $test_blddir
else
...
run_pack() {
	...
	do
		...
		then
			...
			local status=$(echo $?)
			[ $status -eq 0 ] && { /home/user/-f2fs/oe/build.OE-Generic.x86_64-9.0-devel/toolchain/bin/host-gcc -Wall -Wextra -o "$outc.out" "$outc" 2>>"$outc.stderr" || status=2; }

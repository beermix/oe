...
post_unpack() {
  mv $ROOT/$BUILD/${PKG_NAME}-oss-${PKG_VERSION} $ROOT/$BUILD/$PKG_NAME-$PKG_VERSION
  cd $ROOT/$PKG_BUILD
...
makeinstall_target() {
  ...
  cp $ROOT/$PKG_BUILD/out/libmakemkv.so.* $INSTALL/usr/lib
  cp $ROOT/$PKG_BUILD/out/libdriveio.so.* $INSTALL/usr/lib
  cp $ROOT/$PKG_BUILD/out/libmmbd.so.* $INSTALL/usr/lib
}
